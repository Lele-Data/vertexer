...
class RecManager : public TObject{
 public:
  static RecManager *GetInstance(double deltaPhi,double zBinWidth,double deltaZ,double zWidth,int meanNoiseSoft);    // singleton class method
 ...
 private:
  ...
  RecManager(double deltaPhi,double zBinWidth,double deltaZ,double zWidth,int meanNoiseSoft);                        // (private) standard constructor
  ~RecManager();                                                                                                     // (private) destructor
  void Smearing(Point2D *hit,Layer *layer) const;                                                                    // apply smearing to hit points
  int GenerateNhitNoise() const;                                                                                     // generate the number of false hits
  ...
  int fMeanNoiseSoft;                                                                                                // mean number of noise hits
