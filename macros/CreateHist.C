...
void CreateHist(std::string inFilename="recResult",std::string outFilename="HistResult_Eff_Reso"){
  ...
  hZtrueMultRes->GetZaxis()->SetTitle("Z_{rec}-Z_{true} (#mu m)");
  ...
  TH1D *hMultNrec=hZtrueMultNrec->ProjectionX("hMultNrec",1,kNmultBins);
  TH1D *hZtrueNsim=hZtrueMultNsim->ProjectionX("hZtrueNsim",1,kNmultBins);
  TH1D *hMultNsim=hZtrueMultNsim->ProjectionX("hMultNsim",1,kNmultBins);
  ...
  for(int iMult=1;iMult<=kNmultBins;iMult++){
    ...
    sprintf(histTitleRes,"Residues, %5.1f #leq mult < %5.1f",hZtrueMultRes->GetYaxis()->GetBinLowEdge(iMult),hZtrueMultRes->GetYaxis()->GetBinUpEdge(iMult));
    TH1D *projectionOnRes=hZtrueMultRes->ProjectionZ(histNameRes,iMult,iMult,1,kNzTrueBins);
    projectionOnRes->SetTitle(histTitleRes);
  ...
  c1->cd();
  projectionOnZ->Draw();
  TCanvas *c2=new TCanvas("PE");
  c2->cd();
  projectionOnMult->Draw("PE");*/
